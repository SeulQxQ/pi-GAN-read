...
# real_dir: EvalImages/CelebA_real_images_128
...
def output_images(generator, input_metadata, rank, world_size, output_dir, num_imgs=2048):
    ...
    with torch.no_grad():
        while img_counter < num_imgs:
            ...
            generated_imgs, _ = generator.module.staged_forward(z, **metadata) # 生成图像
...
# 计算FID real_dir: EvalImages/CelebA_real_images_128 generated_dir: carla/evaluations/generated
def calculate_fid(dataset_name, generated_dir, target_size=256):
    real_dir = os.path.join('EvalImages', dataset_name + '_real_images_' + str(target_size))
    fid = fid_score.calculate_fid_given_paths(paths=[real_dir, generated_dir], batch_size=128, device='cuda', dims=2048)
