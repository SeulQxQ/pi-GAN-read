...
def load_images(images, curriculum, device):
    ...
    for stage in curriculum['stages']:
        ...
        stage_images = F.interpolate(stage_images, size=stage['img_size'],  mode='bilinear', align_corners=True, recompute_scale_factor=True)
...
def z_sampler(shape, device, dist): # 随机噪声
    if dist == 'gaussian':
...
def train(rank, world_size, opt):
    ...
    curriculum = getattr(curriculums, opt.curriculum) # 获取参数

    # batch_size = 9 img_size = 32 batch_split = 3
    metadata = curriculums.extract_metadata(curriculum, 0) # 提取一轮的参数

    # 噪声输入 输入到 mapping network 中
    fixed_z = z_sampler((25, 256), device='cpu', dist=metadata['z_dist'])  # fixed_z.shaep: [25, 256]
    logging.info(f"fixed_z.shape: {fixed_z.shape}") # [25, 256]

    SIREN = getattr(siren, metadata['model']) # model : TALLSIREN
    ...
    if metadata.get('unique_lr', False):
        ...
        optimizer_G = torch.optim.Adam(generator_ddp.parameters(), lr=metadata['gen_lr'], betas=metadata['betas'], 
                                       weight_decay=metadata['weight_decay']) # 生成器优化器

    optimizer_D = torch.optim.Adam(discriminator_ddp.parameters(), lr=metadata['disc_lr'], betas=metadata['betas'], 
                                   weight_decay=metadata['weight_decay']) # 判别器优化器
    ...
    generator.set_device(device) # generators.py --> set_device() generator implicitGnerator3d FiLM layer
    ...
    torch.manual_seed(rank) # 随机种子
    dataloader = None
    total_progress_bar = tqdm(total = opt.n_epochs, desc = "Total progress", dynamic_ncols=True) # 创建循环和其他迭代过程的进度条
    total_progress_bar.update(discriminator.epoch)
    interior_step_bar = tqdm(dynamic_ncols=True)
    for _ in range (opt.n_epochs): # 训练 n_epochs = 3000
        total_progress_bar.update(1)

        metadata = curriculums.extract_metadata(curriculum, discriminator.step) # 提取一轮的参数

        # Set learning rates 设置学习率
        ...
        # 数据加载器
        if not dataloader or dataloader.batch_size != metadata['batch_size'] or dataloader.dataset.img_size != metadata['img_size']:
            dataloader, CHANNELS = datasets.get_dataset_distributed(metadata['dataset'],
                                        world_size,
                                        rank,
                                        **metadata)
            # discriminator.step: 取curriculum 的第 i(0{}, 100{}, 550{}，2000{}) 个阶段的参数
            ...
            # step_next_upsample: 下一个阶段的轮数 step_last_upsample: 上一个阶段的轮数  [0, int(10e3), int(55e3), int(200e3)]
            interior_step_bar.reset(total=(step_next_upsample - step_last_upsample)) # 两个阶段的轮数差 作为进度条的总数
            interior_step_bar.set_description(f"Progress to next stage")
            interior_step_bar.update((discriminator.step - step_last_upsample))

        for i, (imgs, _) in enumerate(dataloader): # 读取图像数据
            # imgs.shape: [9, 3, 32, 32] [batch_size, channels, img_size, img_size]
            logging.info(f"train imgs.shape: {imgs.shape}") # [9, 3, 32, 32]

            if discriminator.step % opt.model_save_interval == 0 and rank == 0:
                # 每隔 model_save_interval 个轮次保存模型
                now = datetime.now()
            ...
            metadata = curriculums.extract_metadata(curriculum, discriminator.step) # (curriculum, current_step)
            
            if dataloader.batch_size != metadata['batch_size'] or dataloader.dataset.img_size != metadata['img_size']: break
            ...
            alpha = min(1, (discriminator.step - step_last_upsample) / (metadata['fade_steps'])) # fade_steps = 10000

            real_imgs = imgs.to(device, non_blocking=True) #  [9, 3, 32, 32] [batch_size, channels, img_size, img_size]
            ...
            with torch.cuda.amp.autocast():
                ...
                with torch.no_grad():
                    z = z_sampler((real_imgs.shape[0], metadata['latent_dim']), device=device, dist=metadata['z_dist']) # 随机噪声 z [9, 256]
                    logging.info(f"train discriminator z.shape: {z.shape}") # [9, 256]
                    split_batch_size = z.shape[0] // metadata['batch_split'] # real_imgs.shape[0] == z.shape[0] --> batch_size(9)
                    gen_imgs_film = []
                    gen_imgs = []
                    gen_positions = []
                ...
                # 真实图像的 预测结果
                r_preds, _, _ = discriminator_ddp(real_imgs, alpha, **metadata) # --> discriminator.py --> ProgressiveEncoderDiscriminator --> forward()
            ...
            if metadata['r1_lambda'] > 0: # 梯度下降
                ...
                """
                for p grad_real:  # grad_real: [batch_size, 3, img_size, img_size]
                    grad_real.append(p * inv_scale)[0]
                """
            with torch.cuda.amp.autocast():
            ...
            scaler.scale(d_loss).backward() # --> getdatasets 继续 生成 应该是
            ...
            z = z_sampler((imgs.shape[0], metadata['latent_dim']), device=device, dist=metadata['z_dist']) # 随机噪声 z.shape [batch_size, 256]
            ...
            for split in range(metadata['batch_split']):
                with torch.cuda.amp.autocast():
                    ...
                    g_preds, g_pred_latent, g_pred_position = discriminator_ddp(gen_imgs, alpha, **metadata) # --> ProgressiveEncoderDiscriminator --> forward()
            ...
            ema.update(generator_ddp.parameters()) # 更新平均值
            ...
            if rank == 0:
                ...
                if i%10 == 0:
                    tqdm.write(f"[Exp: {opt.output_dir}] [GPU: {os.environ['CUDA_VISIBLE_DEVICES']}] \
                               [Epoch: {discriminator.epoch}/{opt.n_epochs}] [D loss: {d_loss.item()}] \
                               [G loss: {g_loss.item()}] [Step: {discriminator.step}] [Alpha: {alpha:.2f}] \
                               [Img Size: {metadata['img_size']}] [Batch Size: {metadata['batch_size']}] \
                               [TopK: {topk_num}] [Scale: {scaler.get_scale()}]")
            ...
            if opt.eval_freq > 0 and (discriminator.step + 1) % opt.eval_freq == 0:
                ...
                if rank == 0:
                    fid_evaluation.setup_evaluation(metadata['dataset'], generated_dir,  # generated_dir = 'evaluation/generated'
                                                    data_path=metadata["dataset_path"], target_size=128)
                dist.barrier()
                ema.store(generator_ddp.parameters())
                ema.copy_to(generator_ddp.parameters())
                generator_ddp.eval()
                fid_evaluation.output_images(generator_ddp, metadata, rank, world_size, generated_dir) # 评价生成器生成的图像
                ema.restore(generator_ddp.parameters())
                dist.barrier()
                if rank == 0:
