...
class CelebA(Dataset):
    ...
    def __init__(self, dataset_path, img_size, **kwargs):
        ...
        self.data = glob.glob(dataset_path) # glob.glob()返回所有匹配的文件路径列表
        self.img_size = img_size
        assert len(self.data) > 0, "Can't find data; make sure you specify the path to your dataset"
        self.transform = transforms.Compose([transforms.Resize(320), transforms.CenterCrop(256), 
...
class Carla(Dataset):
    ...
    def __getitem__(self, index):
        X = PIL.Image.open(self.data[index]) # 读取图像 size: (512, 512)
        X = self.transform(X) # shape (3, 32, 32)
...
def get_dataset_distributed(name, world_size, rank, batch_size, **kwargs):
    dataset = globals()[name](**kwargs) # 取出数据集 (1w)
